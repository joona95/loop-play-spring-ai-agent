...
- 처음엔 환불+컴플레인처럼 의도가 섞인 질문을 우선순위로 분류하는 tie-break 규칙을 프롬프트에 넣었는데 잘 먹히지 않았다(지금은 뺀 상태). **프롬프트로 안 되면 이런 분류 우선순위를 어떻게 통제해야 하나?**
...
- 프롬프트 통제가 없거나 약하면 모델 언어를 따라가는지 **중국어로 답이 오는 경우**가 있었다. 이런 언어 드리프트도 통제 범위에 명시적으로 넣어야 하나?
